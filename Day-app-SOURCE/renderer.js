...
document.getElementById('submit-button').addEventListener('click', () => {
  ...
  if (nameInput) {
    ...
    // Add click handler for the timetable button
    newButton.addEventListener('click', () => {
      const timeTable = document.querySelector('.time-table');
      const timeTableTitle = timeTable.querySelector('h2');
      timeTableTitle.textContent = nameInput;
      
      // Show timetable with calendar
      timeTable.style.display = 'block';
      
      // Initialize calendar if not already done
      if (!timeTable.hasAttribute('data-calendar-initialized')) {
        const calendar = new Calendar({
          calendarId: 'calendar',
          ...
          onWeekSelect: (selectedWeek) => {
            ...
            // Here you can update the timetable based on selected week
          }
        });
        calendar.init();
        timeTable.setAttribute('data-calendar-initialized', 'true');
      }
    });
...
// Initialize calendar in the timetable
document.addEventListener('DOMContentLoaded', () => {
  const calendar = new Calendar({
    calendarId: 'calendar',
    titleId: 'calendar-title',
    onWeekSelect: (selectedWeek) => {
      // Handle week selection here if needed
      console.log('Selected week:', selectedWeek);
    }
  });
  calendar.init();

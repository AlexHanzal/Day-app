...
function updateTimetableForWeek(newSelectedDate) {
  ...
  dayRows.forEach((row, index) => {
    ...
    cells.forEach((cell, colIndex) => {
      ...
      // Set content from storage for this specific date
      const dateKey = formatDateKey(currentDate);
      newCell.textContent = timetableData[dateKey]?.[colIndex] || '';
    ...
    tableCells.forEach(cell => {
      ...
      cell.addEventListener('input', function() {
        ...
        if (content !== '') {
          ...
          // Save to all weeks
          saveToAllWeeks(rowIndex, colIndex, content);
        } else {
...
function saveCellContent(date, columnIndex, content) {
  ...
  timetableData[dateKey][columnIndex] = content;
}

function getCellContent(date, columnIndex) {
  const dateKey = formatDateKey(date);
  return timetableData[dateKey]?.[columnIndex] || '';
}
